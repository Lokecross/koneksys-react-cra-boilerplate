...
export default class User implements IUser {
  ...
  public isAuth(): boolean {
    ...
    const remainExpirationDays =
      (this.expirationTokenTime - today.getTime()) / oneDay;
  ...
  public needRefreshToken(): boolean {
    const today = new Date();
    const oneDay = 1000 * 60 * 60 * 24;
    const remainExpirationTokenDays =
      (this.expirationTokenTime - today.getTime()) / oneDay;

    const remainExpirationSubscriptionDays =
      (this.subscriptionValidityTime - today.getTime()) / oneDay;
    return (
      remainExpirationTokenDays <= 3 && remainExpirationSubscriptionDays >= 7
    );
  }
  ...
  static fromApiModel(apiModel: IUserApiModel) {
    const expirationTokenTime = new Date(
      Date.parse(apiModel.expiration),
    ).getTime();
    const subscriptionValidityTime = new Date(
      Date.parse(apiModel.subscriptionValidity),
    ).getTime();
    const { role } = apiModel;
